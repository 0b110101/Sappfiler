...
| `别名` | 多选 或 文本 | 可选 | 辅助匹配（游戏名有多种叫法时有用） |
| `游戏标识` | 多选 或 文本 | **推荐** | 填 `steam:appid`（如 `steam:3112010`），可精确匹配运行中的 Steam 游戏 |

> **关于 `游戏标识`**：它只存在于**总表**，用于把本地进程认到总表条目上
> （Steam 游戏的中文名和进程名往往对不上，靠 AppID 才能准确识别）。
> **程序不会把它写进每日时长表**——每日表通过 `游戏` 这个 relation 回指总表，不需要冗余一份。
> 不填也能用，程序会退而从封面 URL 里提取 Steam AppID。
...
Rollup 需要一个「反向关联」，所以先让 `游戏` 这个 relation 双向可见：

1. 打开**每日时长表**，点 `游戏` 属性表头 → **编辑属性**（Edit property）
...
- 检查 Rollup 的 Property 选的是 `时长`、Calculate 选的是 `Sum`
