...
### 问题 7 — 主流平台检测器的"静默漏识别"风险（⚠️ 已修两处，机制性风险仍在）

**设计（用户确认，不要改）**：**只对主流平台（Steam / Epic / GOG / Ubisoft / EA / Xbox / WeGame）
按安装路径自动识别，其余游戏一律靠用户手动「添加游戏」。**
所以"未识别进程被忽略"**不是 bug，是有意为之** —— 早期分析曾误判为设计缺口，已纠正。

**但这条设计有一个硬前提：主流平台的检测器必须准确。**
一旦某个检测器漏了游戏，用户看到的是"我明明在玩 Steam 游戏，程序却毫无反应"，
而且**日志里什么线索都没有**（原因见下）。这类失效已经真实发生过两次：

1. **`SteamDetector` 的 `StateFlags` 当枚举比**（2026-09-18 QA 实测，已修）
   `StateFlags` 是**位标志**：`1`=已卸载 `2`=需更新 `4`=已完整安装
   `8/16/32/64/128`=更新相关 `256`=文件缺失 `1024`=较新状态位。
   常见组合 `4` / `6`(4|2) / `1030`(1024|4|2)。
   旧代码 `flags != "4"` 会丢掉 6/1030 这类**已装好的游戏**。
   本机实测：`4` 是「Steamworks Common Redistributables」（运行库），
   `1030` 是「Where Winds Meet」（真游戏）→ **收下运行库、丢掉真游戏**。
   正确写法：`(flags & 4) != 0 && (flags & 1) == 0`。

2. **`GogDetector` / `UbisoftDetector` 没处理 32 位注册表视图**（2026-09-18 审计发现，已修）
   GOG Galaxy 和 Ubisoft Connect 都是 **32 位程序**，在 64 位 Windows 上写
   `HKLM\SOFTWARE\...` 会被重定向到 `WOW6432Node\SOFTWARE\...`；
   而本程序是 **x64**，`Registry.LocalMachine.OpenSubKey` 默认读 64 位视图 → **完全看不到游戏**。
   已改为两个视图都试（`RegistryKey.OpenBaseKey(hive, view)`）。
   - `SteamDetector` 早就在循环两个视图，`WeGameDetector` 也显式列了 `WOW6432Node`
     —— 说明作者知道这个坑，只有 GOG / Ubisoft 漏了。
   - **Epic / EA / Xbox 不受影响**：它们读的是 `%ProgramData%` 下的文件目录，不碰注册表。

**为什么这两次都很难查（重要教训）**：
- `GameLibraryManager.Refresh()` 原先用 **`Console.WriteLine`**，而这是 **WinExe 无控制台**程序
  —— 输出**直接进虚空**。日志里只有 Notion 报错，检测部分一片空白，只能靠翻源码猜。
  已改用 `AppLog`，并给监控循环补了「开始/结束计时」日志（心跳不记，否则 5 秒一条刷爆）。
- 日志里**不会**出现"某进程被跳过了"这类信息（设计上就是静默忽略）。
  排查"游戏没被计时"时，**第一步先看日志有没有 `[游戏库] xxx: 找到 N 个游戏`**：
  - 该平台 `N = 0` 但用户确实装了游戏 → 检测器有问题，去查那个 detector
  - `N > 0` 但游戏不在其中 → 检测器的过滤条件过严（同 StateFlags 那类问题）
  - `N > 0` 且游戏在其中，但日志没有 `[计时] 开始记录` → 问题在进程侧
    （`ProcessFilter` 误杀、或 exe 路径取不到）
